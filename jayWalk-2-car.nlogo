...
to setup-cars
  ...
  while [count cars < total-vehicles] [
    ...
    ifelse random-float 1 < 0.5 [
      ...
      create-car-in-position [pxcor] of pos [pycor] of pos 90 red
...
;      ]
      ...
      let pos one-of (patches with [pycor = -5])  ; 选择y坐标为5的车道
      create-car-in-position [pxcor] of pos [pycor] of pos 270 green
...
;        set size 5
;        set color green
;        let pos one-of (patches with [pycor = -5])  ; 选择y坐标为-5的车道
;        setxy [pxcor] of pos [pycor] of pos
;        set heading 270  ; 向左行驶
;        set ycor -5
;      ]
    ]
  ]
...
to go
  ask human [
    ...
    ifelse color = yellow and stopped [
      ...
      ;观察时间到，开始过马路
      if stopped and (ticks - stop-tick >= 3)[

        if pycor < 0 and intend = "none" [ set intend "up" set dest one-of [12 15]]
        if pycor > 0 and intend = "none" [ set intend "down" set dest one-of [-11 -14]]
...
;----------------控制车辆数量
  if count cars < total-vehicles[
    ifelse random-float 1 < 0.5 [
      create-car-in-position -48 5 90 red
    ] [
      create-car-in-position 48 -5 270 green
    ]
  ]
...
;-----------------红绿灯控制
 ; 判断是否需要切换红绿灯状态
  if traffic-light-state = "red" [
    if ticks >= last-change-tick + red-light-time [
